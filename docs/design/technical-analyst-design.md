...
Identify and explain the price behavior of each candidate symbol heading into its earnings event — trend, key levels, and positioning — as observable evidence for downstream stages.
...
- Describe how the stock is positioned heading into earnings (e.g., relative to its levels and trend).
- Report the candidate's historical price behavior around past earnings events, where the data provides it (see Open Design Question 2).
...
One technical assessment per candidate per pipeline execution, with a stable output contract. The assessment always contains exactly these sections, in this order:

1. **Trend Assessment** — the prevailing trend and its strength.
2. **Facts** — objective observations only (prices, levels, volumes, dated events), stated without interpretation.
...
6. **Earnings Positioning** — how the stock is positioned heading into the event, including historical earnings price behavior where provided.
...
2. Identify the trend, levels, patterns, and positioning the facts indicate.
...
- Historical earnings behavior is reported as observed distribution of past outcomes, never as a prediction of the upcoming event.
- No directional forecast of the earnings outcome — positioning describes where the stock is, not where it will go.
...
2. **Historical earnings behavior ownership** — this design assigns the candidate's past earnings price reactions to the Technical Analyst (workflow.md step 3 implies it via "how the stock is positioned heading into earnings"; the strategist's prompt expects it from the technical assessment). Confirm this ownership, or assign it elsewhere, before prompt authoring.
